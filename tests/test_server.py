...
from sqlalchemy.exc import IntegrityError, HasDescriptionCode
...
def test_register_happy_path(mocker) -> None:  # type: ignore
    mocker_value = User(
        user_id=uuid.UUID('74f3cde6-0c2d-4222-8b60-6ede5d0a661a'),
        user_name='Andrei',
    ...
    mocker.patch.object(
        Session, "add", return_value=mocker_value
    )
...
def test_authorization_happy_path(mocker) -> None:  # type: ignore
    ...
    mocker.patch.object(
        UsersRepo, "login_user", return_value=mocker_value
    )
...
def test_register_account_already_exists(mocker) -> None:
    mocker.patch.object(
        Session, "add", side_effect=IntegrityError(
            ...
            params={
                ...
                'user_name': 'Andrei',
                'password_hash': b'$2b$12$i6aWPnv1c8HbmJmBWxxHi.kVNGkJBRw2asaSjWwObsJ4ECLik4MBe'
            }
        )
    )
    response = client.post("/register", json={
